...
# def send_message(type, email_address, first_name, last_name, token = None):
#     jsonPayload = json.dumps({'type': type, 
...
#                             })
#     print(jsonPayload)
#     channel.basic_publish(exchange='', routing_key='email', body=jsonPayload )
...
class RabbitMQSender:
  ...
  async def send_message(self, message):
    ...
    await self.channel.default_exchange.publish(message_obj, routing_key=self.queue.name)
...
sender = RabbitMQSender(settings.RABBITMQ_CONNECTION_STRING, settings.RABBITMQ_QUEUE_NAME)
